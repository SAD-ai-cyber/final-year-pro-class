...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = mysqli_prepare($con, "DELETE FROM add_online_students WHERE Student_Id = ?");
    mysqli_stmt_bind_param($stmt, "i", $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);

    // Page refresh kar rega
    header("Location: show-online-student-details.php");
}
...
if (isset($_POST['save_btn'])) {
    ...
    // Update query 
    $update_query = "UPDATE add_online_students SET 
                    student_name =?, 
                    student_email =?, 
                     student_num =? 
    ...
    mysqli_stmt_bind_param($stmt, "sssi", $s_name, $s_email, $s_num, $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
...
$query = "SELECT * FROM add_online_students";
...
<head>
    <title>Online Student Details</title>
...
<body class="fixed-scroll-page">
    <div class="card fixed-scroll-card">
        <div class="card-header fixed-scroll-header">
            <h2 class="card-title">Online Student Details</h2>
        </div>
                    ...
                    <tbody>
                        ...
                        if ($total > 0) {
                            ...
                            <td class='action-buttons'>
                                ...
                                <a href='show-online-student-details.php' title='Cancel' class='btn-cancel'>
                            ...
                            <td class='action-buttons'>
                                <a href='show-online-student-details.php?edit_id=" . $result['Student_Id'] . "' class='btn-edit' title='Edit'>
                                    <i class='fa-solid fa-pen-to-square'></i>
                                </a>

                                <a href='show-online-student-details.php?delete_id=" . $result['Student_Id'] . "&csrf_token=" . urlencode($csrf_token) . "' class='btn-delete' onclick='return confirm(\"Are you sure you want to delete?\")' title='Delete'>
